...
def main() -> int:
    ...
    if not hasattr(lib, "selftest_native_elf_gemv_i16"):
        print("  [FAIL] symbol        selftest_native_elf_gemv_i16 absent (RI-B2-S6 not built)")
        return 1
    print("  [PASS] symbol        selftest_native_elf_gemv_i16 exported by the self-host .so")

    with tempfile.TemporaryDirectory() as td:
        tmp = pathlib.Path(td)
        elf = mind_gemv_i16_elf(lib, ROWS, COLS)
        if not (len(elf) > 120 and elf[:4] == b"\x7fELF"):
            print(f"  [FAIL] elf-image     gemv_i16({ROWS}x{COLS}): not a runnable ELF (len={len(elf)})")
            return 1
        print(f"  [PASS] elf-image     gemv_i16({ROWS}x{COLS}): runnable ELF (len={len(elf)})")
        ...
        if ok:
            ...
            print("native-ELF int16 GEMV is BYTE-IDENTICAL to the MLIR canary "
        ...
        print("  [FAIL] byte-identity  "
              "native-ELF int16 GEMV hash != canary gemv-i16 — reduction/order "
